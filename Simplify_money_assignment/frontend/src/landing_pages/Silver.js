...
function Silver() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/prices/history/silver?timeframe=${timeframe}`)
      .then(res => res.json())
      .then(data => {
        if (!data.error) {
            ...
            <div className="col-lg-8">
                  ...
                  <AreaChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    <defs>
                      <linearGradient id="colorSilver" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="#a8a8bb" stopOpacity={0.1} />
                        <stop offset="95%" stopColor="#a8a8bb" stopOpacity={0} />
                      </linearGradient>
                    </defs>
              ...
              <div className="d-flex justify-content-between my-3 overflow-auto">
                {timeframes.map((tf) => (
                  <button
                    key={tf}
                    className={`btn btn-sm ${timeframe === tf ? 'btn-secondary' : 'btn-outline-secondary'} mx-1`}
            ...
            <div className="col-lg-4 mb-3 mb-lg-0">
              <div className="border rounded p-3 shadow-sm bg-white d-flex flex-column justify-content-center" style={{ height: "350px" }}>
                ...
                <p className="text-muted" style={{ lineHeight: "1.6" }}>
                  Silver offers a unique dual value profile: it is universally recognized as a precious metal and store of wealth, but it also has massive industrial demand, especially in renewable energy and electronics.
                </p>
                <p className="text-muted" style={{ lineHeight: "1.6" }}>
                  Because it's more accessible than gold, silver presents a fantastic entry point for investors looking to protect their portfolio against inflation while capitalizing on future industrial growth.
                </p>
              </div>
            </div>
