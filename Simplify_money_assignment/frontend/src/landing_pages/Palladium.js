...
function Palladium() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/prices/history/palladium?timeframe=${timeframe}`)
      .then(res => res.json())
      .then(data => {
        if (!data.error) {
  ...
  return (
    <div className="container p-4">
      <h2 className="mb-2">Palladium</h2>
            ...
            <div className="col-lg-8">
              <div style={{ width: "100%", height: 350 }} className="border rounded p-3 shadow-sm bg-white h-100">
                <ResponsiveContainer>
                  <AreaChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    <defs>
                      <linearGradient id="colorPalladium" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="#bdc3cb" stopOpacity={0.1} />
                        <stop offset="95%" stopColor="#bdc3cb" stopOpacity={0} />
                      </linearGradient>
                    </defs>
              ...
              <div className="d-flex justify-content-between my-3 overflow-auto">
                {timeframes.map((tf) => (
                  <button
                    key={tf}
                    className={`btn btn-sm ${timeframe === tf ? 'btn-info text-white' : 'btn-outline-secondary'} mx-1`}
            ...
            <div className="col-lg-4 mb-3 mb-lg-0">
              <div className="border rounded p-3 shadow-sm bg-white d-flex flex-column justify-content-center" style={{ height: "350px" }}>
                ...
                <p className="text-muted" style={{ lineHeight: "1.6" }}>
                  Palladium plays a crucial, irreplaceable role in modern gasoline vehicles by neutralizing harmful emissions. Its supply is highly concentrated and frequently faces deficits.
                </p>
                <p className="text-muted" style={{ lineHeight: "1.6" }}>
                  Investing in palladium capitalizes on strict environmental regulations and tight supply chains, offering substantial upside potential distinct from gold or silver markets.
                </p>
              </div>
            </div>
